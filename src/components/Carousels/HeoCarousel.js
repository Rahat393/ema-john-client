  ...
  return (
    <div className='slider'>
            {content.map((item, index) => <div key={index} className={`slide ${slideNumber === (index + 1) && 'active'}`}>
                ...
                <div className='info lg:ml-20'>
                    <h1 className='lg:text-5xl mb-4 md:text-4xl text-2xl font-bold uppercase text-white'>{item.title}</h1>
                    <p className='mt-2 lg:mb-8 md:mb-8 mb-5 lg:text-lg md:text-lg text-sm lg:w-3/4 md:w-3/4'>{item.description}</p>
                    
                    <Link to='/allproduct'><ButtonPrimary>Shop Now
                    </ButtonPrimary></Link>
